...
class RDFParser {
	...
	private static int parse(CharBuffer line) throws ParseException {
		...
		// it's a value
			...
			if (myValue.charAt(1) != 'A') {
				...
				// timestampIndex+"," + timestampValue + "," + value);
	...
	static String tmp;
	static long timestampIndextemp;

	public static void checkIt(long timestampIndex2, String timestampValue) {

		if (!tmp.equals(timestampValue)){
			if (timestampIndex2 != timestampIndextemp) {
				System.out.println(timestampIndex2 +"  -- "+ timestampValue);
			} else
				timestampIndextemp = timestampIndex2;
		} else
			tmp = timestampValue;
	}
}
